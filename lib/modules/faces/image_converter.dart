...
imglib.Image? cameraImageToImage(CameraImage cameraImage) {
  try {
    if (cameraImage.format.group == ImageFormatGroup.yuv420) {
      return convertYUV420toImageColor(cameraImage);
...
imglib.Image convertYUV420toImageColor(CameraImage image, {bool isRotation = false}) {
  ...
  var img = isRotation
      ? imglib.Image(width: height, height: width)
  ...
  for (int x = 0; x < width; x++) {
    for (int y = 0; y < height; y++) {
      ...
      if (img.isBoundsSafe(height - y, x)) {
        img.setPixelRgba(height - y, x, r, g, b, (0xFF << 24));
      }
    }
  }
  return img;

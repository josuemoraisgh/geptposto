...
class AssistidosStoreList {
  ...
  AssistidosStoreList(
    ...
    this.configStore =
        configStore ?? Modular.get<ConfigStorageInterface>();
    this.remoteStore =
        remoteStore ?? Modular.get<RemoteStorageInterface>();
    ...
    if (isRunningSync == false) {
      ...
      if (remoteConfigChanges != null && remoteConfigChanges.isNotEmpty) {
        for (List e in remoteConfigChanges) {
          ...
          configStore.addConfig(e[0], e.sublist(1).cast<String>());
        }
      }
      ...
      if (image != null) {
        ...
        if (faceDetected.isNotEmpty) {
          image = isUpload
              ...
              : image;
          fotoPoints = (await faceDetectionService.classificatorImage(image));
        }
        stAssist.photo = [
          photoFileName,

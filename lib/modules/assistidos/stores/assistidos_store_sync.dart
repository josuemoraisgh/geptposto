...
class AssistidosStoreSync {
  late final AssistidosStore assistidosStore;
  AssistidosStoreSync({AssistidosStore? assistidosStoreAux}) {
  ...
  Future<void> init() async {
    ...
    itensListController = assistidosStore.configStore
        .watch("itensList")
        .asBroadcastStream() as Stream<BoxEvent>;
    assistidosStore.syncStore.addListener(sync);
    sync();
  }
    ...
    if (isRunningSync == false) {
      ...
      if (remoteDataChanges != null) {
        for (var e in remoteDataChanges) {
          ...
          assistidosStore.localStore.setRow(stAssist).then(
            (value) async {
              getPhoto(stAssist);
            },
          );
